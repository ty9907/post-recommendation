...
/**
 * IKAnalyzerTagExtractor测试类
 * 测试IKAnalyzer标签提取器的各种功能
 */
public class IKAnalyzerTagExtractorTest {
    private static final Logger logger = LoggerFactory.getLogger(IKAnalyzerTagExtractorTest.class);
    ...
    public void testExtractTags() {
        ...
        assertEquals("IKAnalyzerTagExtractor", extractor.getName());
        
        logger.info("=== IKAnalyzerTagExtractor 提取的标签 ===");
    ...
    public void testExtractTagsWithLimit() {
        ...
        logger.info("=== IKAnalyzerTagExtractor 提取的标签 (限制5个) ===");
        for (int i = 0; i < tags.size(); i++) {
            Tag tag = tags.get(i);
            logger.info("{}. {} (权重: {}, 频率: {})", 
                    (i + 1), tag.getName(), String.format("%.4f", tag.getWeight()), tag.getFrequency());
    ...
    public void testTagOrdering() {
        ...
        logger.info("=== IKAnalyzerTagExtractor 标签排序测试 ===");
        for (int i = 0; i < tags.size(); i++) {
            Tag tag = tags.get(i);
            logger.info("{}. {} (权重: {}, 频率: {})", 
                    (i + 1), tag.getName(), String.format("%.4f", tag.getWeight()), tag.getFrequency());
