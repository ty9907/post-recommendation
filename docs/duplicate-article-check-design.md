...
### 1.4 当前实现状态（2026-03-22 更新）

本设计文档对应的核心能力现已在代码中落地，且在原有实现基础上补齐了性能优化链路：

- 已实现富文本预处理接口与默认实现，支持 HTML/XML 标签移除、实体解码和文本规范化
- 已实现 SimHash 倒排索引与标签倒排索引，用于分层候选筛选
- 已实现候选集管理器，支持 SimHash -> 标签 的分层过滤与候选缓存
- 已实现风险等级评估与异步精检服务，用于发布前快速检测后的后续处理
- 已实现性能监控与索引同步服务，支持记录阶段耗时、候选集规模以及索引重建
- `RealTimeDetector` 和 `BatchDetector` 已扩展为支持分层检测流程

需要说明的是：

- 当前仓库仍然以“库/SDK”形式提供能力，不包含完整的 Web API 或 Spring Boot 外层服务
- `ArticleRepository` 已补充为对外接入接口，具体数据存取实现仍由业务系统负责
